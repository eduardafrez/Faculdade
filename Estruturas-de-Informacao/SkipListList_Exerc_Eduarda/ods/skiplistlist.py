...
# índices i, . . . , n − 1. Esse método deve ser executado em um tempo O(log n).
    def truncate(self, i):
        if i < 0 or i > self.n:
            raise IndexError("Indice falho")

        # Cria uma nova lista
        l2 = SkiplistList()

        # Ajusta o tamanho da nova lista
        l2.n = self.n - i
        self.n = i  # A lista original perde os elementos a partir do índice i

        # Copiar os elementos a partir do índice i da lista original para a nova lista
        s = self.sentinel
        h = self.h
        a = -1
        while h >= 0:               # Garante de percorrerá toda skiplist
            while s.next[h] is not None and a + s.length[h] < i:        # Verifica se ainda tem nó de nivel h e se o comprimento do salto no nível h+a seja menor que i
                a += s.length[h]                                    # Atualiza a
                s = s.next[h]                                       # Atualiza s
            if a + s.length[h] == i:                            # Caso comprimento do salto no nível h+a seja menor que i
                # A nova lista vai começar a partir deste ponto
                l2.sentinel.next[h] = s.next[h]
                l2.sentinel.length[h] = s.length[h]
                s.next[h] = None  # Limpar a referência na lista original a partir de i
                s.length[h] = i - a  # Redefinir a contagem de elementos restantes na lista original
            h -= 1

        # Ajuste da altura da nova lista, se necessário
        while l2.h > 0 and l2.sentinel.next[l2.h] is None:
            l2.h -= 1

        # garanter que todos os elementos de l2 estão corretamente indexados
        current = l2.sentinel.next[0]
        idx = 0
        while current is not None:
            # A nova lista vai ter seus índices ajustados para começar de 0
            current.x = current.x  # Não altera o valor, só reindexa
